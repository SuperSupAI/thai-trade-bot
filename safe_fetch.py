...
def _download_direct(symbol, years):
...
def _dl_worker(symbol, years, q):
    """รันในโปรเซสลูก (fork) — ถ้า segfault ตรงนี้ กระทบแค่โปรเซสนี้"""
    c = _download_direct(symbol, years)
    q.put(("ok", c))


def safe_download_one(symbol, years, timeout=TIMEOUT_PER_SYMBOL):
    """ดาวน์โหลดหุ้น 1 ตัวแบบปลอดภัย คืน pandas Series หรือ None"""
    if not _HAS_FORK:
        return _download_direct(symbol, years)

    ctx = mp.get_context("fork")
    q = ctx.Queue()
    p = ctx.Process(target=_dl_worker, args=(symbol, years, q))
    p.start()
    p.join(timeout)
