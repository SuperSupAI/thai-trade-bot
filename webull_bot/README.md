# Webull Trading Bot (US100, Trend+MACD + TP12%/SL15%)

บอทสแกนหุ้น US100 ทุกวันหลังตลาดปิด ใช้สูตรที่ทดสอบแล้วใน `test_exit_optimization.py` /
`test_exit_optimization_crashcheck.py` (Close>EMA200 · EMA10>EMA50 · EMA50>EMA200 · MACD>0
เข้า / TP12%+SL15% ออก, ถือพร้อมกัน 10 ไม้) — เลือกสูตรนี้เพราะเป็นสูตรเดียวจาก 690 คอมโบที่ทดสอบ
ที่ยังกำไรได้จริงในตลาดหมี 2022 (+4.7% ขณะ B&H -16.0%) ไม่ใช่เพราะกำไรสูงสุดตอนตลาดขาขึ้น
(สูตรอื่นที่กำไรสูงกว่าตอนขาขึ้นกลับขาดทุนหนักตอนตลาดขาลง — อ่านคำเตือนในแชทก่อนใช้เงินจริง)
...
- **ยังไม่กรองหุ้น sideway ออกจาก universe ก่อนสแกน** — ทดสอบแล้ว (`test_sideways_stocks.py`)
  พบว่าสูตร Trend+MACD ปัจจุบันขาดทุน -6.6% บนหุ้น sideway/choppy (KER ต่ำ) ขณะที่หุ้นเทรนด์
  แข็งแรงกำไร +48.6% ในช่วงเวลาเดียวกัน — แนวทางที่คิดไว้: เช็ค Kaufman Efficiency Ratio
  (KER = |ราคาสุดท้าย-เริ่มต้น| / ผลรวม|เปลี่ยนแปลงรายวัน| ย้อนหลัง ~252 วัน) ก่อนอนุญาตให้เข้าซื้อ
  ตัดหุ้นที่ KER ต่ำกว่า threshold (เช่น <0.05) ออกจาก candidate ใน `bot.py` ก่อนเรียก
  `entry_signal()` — ยังไม่ implement จริง แค่โน้ตแนวทางไว้
