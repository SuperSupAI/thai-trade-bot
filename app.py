...
|---|---|---|---|---|---|---|
| **DR สหรัฐฯ** | 252 วัน (12mo) | 52 ตัว confirmed§ (จาก 95 ตัวที่ติดตาม) | **+2,060.9%** | +222.1% | -31.4% | ไม่มี |
...
§ ตัวเลข DR สหรัฐฯ ในตารางนี้ใช้เฉพาะ 52 ตัวที่มีรหัส DR ยืนยันแล้ว (เทรดได้จริงกับบอทตอนนี้) ไม่รวมอีก 43 ตัว
ที่ยังหารหัส DR ไม่เจอ (unconfirmed) — ถ้ารวมทั้ง 95 ตัว (ทั้ง confirmed+unconfirmed) ตัวเลขจะดีกว่านี้มาก
(ALL +11,243.7%, TEST +325.2%, 2022 +6.0%) แต่เป็นตัวเลข "เพื่อการวิจัย" เท่านั้น เพราะ 43 ตัวนั้นบอทจริงซื้อไม่ได้
...
- **22 ก.ค. 2026 (ต่อ 3)** — ผู้ใช้ส่งลิสต์ 132 หุ้น parent stock 21 ธีมจากอินฟลูเอนเซอร์การเงิน (เม่าปอย/เพจ
  ...
  จริง ไม่ใช่ overfit, ค้นรหัส DR จริงยืนยันได้แค่ 8/48 ตัว (ที่เหลือ 40 ตัวยังเทรดจริงไม่ได้) (ดูหัวข้อ
  🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕 ด้านล่าง)
...
**D) ยืนยันรหัส DR จริง — ได้แค่ 8/48 ตัว**
...
`dr_universe.py` ใส่ระบบ `DR_SYMBOL_MAP` เดิมตรงๆ: `(dr_symbol, "confirmed")` สำหรับ 52 ตัวที่ยืนยันแล้ว,
`(None, "unconfirmed")` สำหรับ 43 ตัวที่เหลือ — บอทจริง (`bot.py`) ใช้ `TRADABLE_TICKERS` filter ที่มีอยู่
แล้ว (กรองเฉพาะตัวที่มี `get_dr_symbol()[0]` ไม่ใช่ None) จึงคัดกรอง 43 ตัว unconfirmed ออกจากการเทรดจริง
...
**E) ข้อควรระวังเชิงปฏิบัติ (เช็ค ณ เวลาทดสอบ)**

ทดสอบ live ranking ผ่าน Monitor page พบว่า **top-3 ปัจจุบันคือ SNDK (+5,078.4%), BE (+1,216.2%), WDC
(+1,000.4%) — ทั้ง 3 ตัวเป็นตัวใหม่ที่ยัง unconfirmed ทั้งหมด** แปลว่าบอทจริง ณ ตอนนี้จะข้ามไปซื้อตัวที่ยืนยัน
รหัส DR แล้วอันดับถัดไปแทน (เช่น MU +892.8%) — เป็นเรื่องคาดหวังได้ตามการออกแบบระบบ (ไม่ใช่บั๊ก) แต่เป็น
ตัวอย่างที่ชัดเจนว่าทำไมต้องแยก "universe เพื่อจัดอันดับ/วิจัย" ออกจาก "universe ที่เทรดได้จริง" ให้ชัดเจน

**สรุป: เพิ่ม 95 ตัวเข้า universe การจัดอันดับ (Monitor/Backtest) แล้ว, บอทจริงเทรดได้เฉพาะ 52 ตัว confirmed
เท่านั้น, ยังเหลืองานหารหัส DR อีก 40 ตัวที่ยังไม่ยืนยัน (ผ่าน set.or.th โดยตรง) เป็นงานถัดไปถ้าอยากได้ผล
เต็ม 95 ตัว**
