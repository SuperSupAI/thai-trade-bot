...
|---|---|---|---|---|---|---|
| **DR สหรัฐฯ** | 252 วัน (12mo) | 65 ตัว confirmed§ (จาก 95 ตัวที่ติดตาม) | **+11,922.5%** | +325.9% | -13.7% | ไม่มี |
...
§ ตัวเลข DR สหรัฐฯ ในตารางนี้ใช้เฉพาะ 65 ตัวที่มีรหัส DR ยืนยันแล้ว (เทรดได้จริงกับบอทตอนนี้) ไม่รวมอีก 30 ตัว
ที่ยังหารหัส DR ไม่เจอ (unconfirmed) — ถ้ารวมทั้ง 95 ตัว (ทั้ง confirmed+unconfirmed) ผลตอบแทน ALL/TEST จะ
ใกล้เคียงกัน (+11,243.7%/+325.2%) แต่ปี 2022 stress **แย่กว่า 65-confirmed** (+6.0% vs -13.7%) เพราะ 30 ตัว
unconfirmed ส่วนใหญ่เป็นกลุ่มกระจายความเสี่ยง (เหมือง/พลังงาน/สุขภาพ) ที่ไม่ร่วงหนักปี 2022 เท่ากลุ่มเทค —
ตัดออกแล้วพอร์ตกระจุกตัวเทคมากขึ้น (ดูหัวข้อขยาย universe เม่าปอย ด้านล่างสำหรับรายละเอียดเต็ม)
...
- **22 ก.ค. 2026 (ต่อ 3)** — ผู้ใช้ส่งลิสต์ 132 หุ้น parent stock 21 ธีมจากอินฟลูเอนเซอร์การเงิน (เม่าปอย/เพจ
  ...
  settrade/pi.financial/bualuang ทีละตัว เจอเพิ่มอีก 13 ตัว (รวม 65/95 confirmed) พร้อมจับได้ว่า "TEL23" ที่
  ดูเหมือนตรงกับ ticker เราคือ Tokyo Electron ไม่ใช่ TE Connectivity ตัวจริง (ไม่ผูกรหัสผิด) (ดูหัวข้อ
  🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕🆕 ด้านล่าง)
...
`dr_universe.py` อัปเดต `DR_SYMBOL_MAP`: `(dr_symbol, "confirmed")` สำหรับ 65 ตัวที่ยืนยันแล้ว,
`(None, "unconfirmed")` สำหรับ 30 ตัวที่เหลือ — บอทจริง (`bot.py`) ใช้ `TRADABLE_TICKERS` filter ที่มีอยู่
แล้ว (กรองเฉพาะตัวที่มี `get_dr_symbol()[0]` ไม่ใช่ None) จึงคัดกรองตัว unconfirmed ออกจากการเทรดจริง
โดยอัตโนมัติ ไม่ต้องแก้โค้ดบอทเพิ่ม — ส่วนหน้า Monitor/Backtest ในแอปนี้ยังคงแสดงครบ 95 ตัวเพื่อการวิจัย
พร้อม banner เตือนตัวที่ unconfirmed

**E) Backtest ด้วย 65-confirmed อัปเดตแล้ว — 2022 stress แย่ลงเล็กน้อยจาก 52-confirmed แต่ ALL/TEST ดีขึ้น**

| | 52 confirmed (รอบแรก) | 65 confirmed (รอบสอง) | 95 ทั้งหมด (รวม unconfirmed) |
|---|---|---|---|
| ALL (10 ปี) | +2,060.9% | **+11,922.5%** | +11,243.7% |
| TEST | +222.1% | **+325.9%** | +325.2% |
| 2022 (stress) | -31.4% | -13.7% | **+6.0%** |

13 ตัวที่เพิ่งยืนยัน (โดยเฉพาะ SNDK/SMCI/MRVL/COHR/LITE ที่เป็นหุ้นเซมิคอนดักเตอร์/เทคกำลังวิ่งแรงปี 2023-25)
ดัน ALL/TEST ขึ้นชัดเจน แต่ 2022 stress แย่ลงกว่า 95-ตัวเต็ม (-13.7% vs +6.0%) เพราะ 30 ตัวที่ยัง unconfirmed
ส่วนใหญ่เป็นกลุ่มกระจายความเสี่ยง (เหมือง/พลังงาน/สุขภาพ) ที่ไม่ร่วงหนักปี 2022 — พอตัดออกพอร์ต confirmed
กระจุกตัวไปทางเทค/เซมิคอนดักเตอร์มากขึ้น เป็น trade-off ที่สมเหตุสมผล ไม่ใช่ข้อผิดพลาด

**F) ข้อควรระวังเชิงปฏิบัติ (เช็ค ณ เวลาทดสอบรอบแรก)**

ทดสอบ live ranking ผ่าน Monitor page (ก่อนรอบสองนี้) พบว่า **top-3 ตอนนั้นคือ SNDK (+5,078.4%), BE
(+1,216.2%), WDC (+1,000.4%) — ทั้ง 3 ตัวตอนนั้นยัง unconfirmed ทั้งหมด** แปลว่าบอทจริงตอนนั้นจะข้ามไปซื้อ
ตัวที่ยืนยันรหัส DR แล้วอันดับถัดไปแทน (เช่น MU +892.8%) — **ตอนนี้ SNDK ยืนยันแล้ว (SNDK03)** จึงน่าจะกลับมา
ซื้อได้จริงถ้ายังติดอันดับ top-3 (BE/WDC ยังไม่ยืนยัน ยังคงถูกข้าม) — เป็นตัวอย่างที่ชัดเจนว่าทำไมต้องแยก
"universe เพื่อจัดอันดับ/วิจัย" ออกจาก "universe ที่เทรดได้จริง" ให้ชัดเจน และทำไมต้องอัปเดต confirmed list
เป็นระยะ

**สรุป: 95 ตัวเข้า universe การจัดอันดับ (Monitor/Backtest) แล้ว, บอทจริงเทรดได้ 65 ตัว confirmed (เพิ่มจาก
52), ยังเหลืองานหารหัส DR อีก 30 ตัวที่ยังไม่ยืนยัน ส่วนใหญ่เป็นหุ้นเล็ก/niche ที่มีโอกาสสูงว่ายังไม่มี DR
จริงในตลาดไทย**
